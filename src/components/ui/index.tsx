...
export const EditorialMetric = ({ value, label, overrideColor = "text-brand-black" }: { value: string, label: string, overrideColor?: string }) => (
  ...
  >
    <span className={`text-5xl md:text-7xl font-serif mb-2 ${overrideColor}`}>{value}</span>
...
export const SupportTag = ({ children }: { children: React.ReactNode }) => (
  <div className="border border-brand-black px-4 py-2 text-xs md:text-sm uppercase tracking-widest font-sans hover:bg-brand-black hover:text-brand-paperLight transition-colors duration-300">
    {children}
  </div>
);
