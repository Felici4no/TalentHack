...
export const ProblemSection = () => (
  <section className="py-24 px-8 bg-brand-black text-brand-offwhite border-b border-brand-black/20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 mb-16">
        {siteData.diagnosisContent.points.map((point, i) => (
          <motion.div 
            ...
            className="border-t border-brand-offwhite/20 pt-6"
          >
            <h3 className="text-xl font-serif mb-2 text-brand-red">— {point.title}</h3>
            <p className="font-sans text-brand-gray-light/80 text-sm leading-relaxed">{point.desc}</p>
          </motion.div>
        ))}
      </div>

      <div className="text-center pt-12 border-t border-brand-offwhite/10">
        <p className="text-2xl font-serif uppercase tracking-widest text-brand-offwhite">{siteData.diagnosisContent.keyMessage}</p>
      </div>
    </div>
  </section>
);
  ...
  <section className="py-24 px-8 bg-brand-gray-light border-b border-brand-black/10">
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row gap-16 mb-24">
        ...
        <p className="text-lg md:text-xl font-sans leading-relaxed flex-1 pt-2 border-l border-brand-black/20 pl-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
        {siteData.capitalCulturalCards.types.map((type, i) => (
          <div key={i} className="border border-brand-black p-8 flex flex-col gap-4 bg-brand-offwhite hover:bg-brand-pink-soft transition-colors duration-300">
